...
# ── Brand colours ────────────────────────────────────────────────────────────
C_PALE   = colors.HexColor("#EEF5FF")   # light background / tinted panels
C_MID    = colors.HexColor("#4B6FBF")   # accent / headings
C_DARK   = colors.HexColor("#081F60")   # primary dark / title
C_WHITE  = colors.white
C_GRAY   = colors.HexColor("#7A8DB3")
C_LGRAY  = colors.HexColor("#C8D6EE")
C_SUCCESS= colors.HexColor("#1D9E75")
C_WARN   = colors.HexColor("#BA7517")
C_INFO   = colors.HexColor("#185FA5")

TARGET_COLORS = {
    "human"  : colors.HexColor("#1D9E75"),
    "animal" : colors.HexColor("#BA7517"),
    "vehicle": colors.HexColor("#185FA5"),
}
...
def cover_page(canv, caps, mission_id):
    ...
    canv.drawString(MARGIN + 6, h * 0.68,
        "Automated hotspot detection  ·  Classification  ·  Auto-capture")
...
def summary_section(story, caps):
    ...
    if map_buf:
        ...
        story.append(Paragraph(
            "Map shows GPS-tagged detection events with flight path. "
            "Dot size reflects detection confidence. Numbers match capture log.",
            STYLES["small"]))
        story.append(Spacer(1, 10))
